...
object Anagrams {
  ...
  occurrences match {
    ...
    case (char, count) :: xs =>
      for {
        combined <- combinations(xs)
        i <- 0 to count
      } yield ((char, i) :: combined).filter(_._2 > 0)
  }


  def combinations_(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((char, count), acc) =>
        acc ++ (for {combination <- acc; i <- 1 to count} yield (char, i) :: combination)
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    def sub(char: Char, count: Int) = {
      yMap.get(char) match {
        case None => (char, count)
        case Some(yCount) => (char, count - yCount)
      }
    }
    x.map(t => sub(t._1, t._2)).filter(t => t._2 > 0)
  }
      ...
      occurrences match {
        ...
        case _ =>
          for {
            ...
            words <- dictionaryByOccurrences.getOrElse(combination, Nil)
            anagram <- anagrams(subtract(occurrences, wordOccurrences(words)))
          } yield words :: anagram
      }
