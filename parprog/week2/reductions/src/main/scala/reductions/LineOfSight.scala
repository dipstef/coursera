...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var maxAngle = 0f
    for (i <- input.indices) {
      maxAngle = max(tangent(input, i), maxAngle)
      output(i) = maxAngle
    }
  }
  ...
  def tangent(input: Array[Float], from: Int) = if (from == 0) 0f else input(from) / from
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
                tree: Tree): Unit = {
    tree match {
      case Leaf(from, end, maxPrevious) =>
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
                     threshold: Int): Unit = {
    downsweep(input, output, startingAngle = 0f, tree = upsweep(input, 1, input.length, threshold))
  }
}
