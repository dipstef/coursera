...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    means.map((_, Nil)).toMap ++ points.groupBy(p => means.reduce((m1, m2) =>
      if (p.squareDistance(m1) < p.squareDistance(m2)) m1 else m2))
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val newMeans = update(classified=classify(points, means), means)
    if (converged(eta)(newMeans, means)) newMeans else kMeans(points, newMeans, eta)
  }
}
